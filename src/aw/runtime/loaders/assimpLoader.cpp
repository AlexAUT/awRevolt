...
bool AssimpLoader::loadFromStream(const std::string& fileName, std::istream& stream, const char* hint)
{
  ...
  LogAssimpE() << "Meshes: " << scene->mNumMeshes << ", tex: " << scene->mNumTextures;
  LogAssimpE() << "Materials: " << scene->mNumMaterials;
  LogAssimpE() << "Animations: " << scene->mNumAnimations;
  LogAssimpE() << scene->mNumAnimations;
  return true;
}
